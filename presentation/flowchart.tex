...
\matrix (m)[matrix of nodes, column  sep=2cm,row  sep=8mm, align=center, nodes={rectangle,draw, anchor=center} ]{
  |[block]| {$\hat{u}_{list} =$ complete permutation $[\hat{u}_1,\hat{u}_2,\hat{u}_3,\hat{u}_4] $}              &  \\
  |[block]| {$min = 100$}              &  \\
    |[block]| {For i in lenght($\hat{u}_{list}$) }               &  \\
    |[block]| {
        $\vec{B} \cdot \hat{u}_{list}[i]_1 = B_1$ \\
        $\vec{B} \cdot \hat{u}_{list}[i]_2 = B_2$ \\
        $\vec{B} \cdot \hat{u}_{list}[i]_3 = B_3$ \\
      }          &                                             \\
      |[block]| {$check_{exp} = \vec{B} \cdot \hat{u}_{list}[i]_3$}    & \\
      |[block]| {percentage difference $ = 2 \cdot \frac{\mid check_{exp} - B_4 \mid}{check_{exp} + B_4}\cdot 100$}    & \\
