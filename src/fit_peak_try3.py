...
def generate_model(spec):
    ...
    for i, basis_func in enumerate(spec['model']):
        ...
        if basis_func['type'] in ['GaussianModel', 'LorentzianModel','InvLorentzianModel', 'VoigtModel']: # for now VoigtModel has gamma constrained to sigma
            ...
            model.set_param_hint('constant', min=0.9, max=1.1)
...
def peak_finder(x):
    contrast = (max(x) - min(x))/(max(x) + min(x))
    height = max(x)*(1-contrast)/(1+contrast)
    height_new = max(x)*(1-contrast*a)/(1+contrast*a)
    print('Contrast:', contrast, '\nHeight:', height, '\nNew height:', height_new)
    print("")
    peak_indicies, properties = signal.find_peaks(x * -1., height = -0.996)
