...
def center_split():
    ...
    popt_ext_up, pcov_ext_up = curve_fit(Deviation_plotter_plus, B, peak_ext_up, sigma= peak_ext_up_err)
    perr_ext_up = np.sqrt(np.diag(pcov_ext_up))
    fit_ext_up = Deviation_plotter_plus(B, *popt_ext_up)
    ax[1].plot(B_arr,fit_ext_up,color='orange', label='['+str("{:.2f}".format(popt_ext_up[0]))+'$\pm$'+str("{:.2f}".format(perr_ext_up[0]))+']'+'$\degree$')
    popt_ext_down, pcov_ext_down = curve_fit(Deviation_plotter_minus, B, peak_ext_down, sigma= peak_ext_down_err)
    perr_ext_down = np.sqrt(np.diag(pcov_ext_down))
    fit_ext_down = Deviation_plotter_minus(B, *popt_ext_down)
    ax[1].plot(B_arr,fit_ext_down,color='orange', label='['+str("{:.2f}".format(popt_ext_down[0]))+'$\pm$'+str("{:.2f}".format(perr_ext_down[0]))+']'+'$\degree$')
