...
def Deviation_plotter():
    ...
    ax.plot(B, freqs[:,0],color='black', label='$0\degree$')
    ax.plot(B, freqs[:,1],color='black')
    ax.axhline(2.87, c='black', linestyle='dotted')
    ax.plot(B, freqs_dev_30[:,0],color='blue', label='$30\degree$')
    ax.plot(B, freqs_dev_30[:,1],color='blue')
    # ax.plot(B, (freqs_dev_30[:,0]+freqs_dev_30[:,1])/2,color='blue', label='$30\degree$')
    ...
    plt.ylabel('$Transition \ Frequencies \ [GHz]$', fontsize=18)
