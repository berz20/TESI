...
def gaussian(x, amplitude=1.0, center=0.0, sigma=1.0):
    ...
    """
    return (amplitude / (max(tiny, s2pi * sigma))) * exp(
        -((1.0 * x - center) ** 2) / max(tiny, (2 * sigma**2))
    )


def gaussian2d(
    x, y=0.0, amplitude=1.0, centerx=0.0, centery=0.0, sigmax=1.0, sigmay=1.0
):
    ...
    """
    z = amplitude * (
        gaussian(x, amplitude=1, center=centerx, sigma=sigmax)
...
def invlorentzian(x, amplitude=1.0, center=0.0, sigma=1.0):
    """Return a 1-dimensional Lorentzian function.
...
def pvoigt(x, amplitude=1.0, center=0.0, sigma=1.0, fraction=0.5):
    ...
    return (1 - fraction) * gaussian(
        x, amplitude, center, sigma_g
    ) + fraction * lorentzian(x, amplitude, center, sigma)


def moffat(x, amplitude=1, center=0.0, sigma=1, beta=1.0):
...
def damped_oscillator(x, amplitude=1.0, center=1.0, sigma=0.1):
    ...
    return amplitude / sqrt(
        (1.0 - (x / center) ** 2) ** 2 + (2 * sigma * x / center) ** 2
    )
...
def logistic(x, amplitude=1.0, center=0.0, sigma=1.0):
    ...
    """
    return amplitude * (1.0 - 1.0 / (1.0 + exp((x - center) / max(tiny, sigma))))


def lognormal(x, amplitude=1.0, center=0.0, sigma=1):
    ...
    if isinstance(x, (int, float)):
        ...
        x[where(x <= tiny)] = tiny
    return (amplitude / (x * max(tiny, sigma * s2pi))) * exp(
        -((log(x) - center) ** 2) / max(tiny, (2 * sigma**2))
    )
...
def students_t(x, amplitude=1.0, center=0.0, sigma=1.0):
    ...
    return (
        amplitude
        * (1 + (x - center) ** 2 / max(tiny, sigma)) ** (-s1)
        * gamfcn(s1)
        / denom
    )
...
def thermal_distribution(x, amplitude=1.0, center=0.0, kt=1.0, form="bose"):
    ...
    if form.startswith("bose"):
        ...
        msg = (
            f"Invalid value ('{form}') for argument 'form'; should be one "
...
def rectangle(
    x, amplitude=1.0, center1=0.0, sigma1=1.0, center2=1.0, sigma2=1.0, form="linear"
):
    ...
    if form == "erf":
        ...
        out = 1.0 - 1.0 / (1.0 + exp(arg1)) - 1.0 / (1.0 + exp(arg2))
    elif form in ("atan", "arctan"):
