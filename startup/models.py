...
def _validate_1d(independent_vars):
    if len(independent_vars) != 1:
        raise DimensionalError("This model requires exactly one independent variable.")
...
def guess_from_peak2d(model, z, x, y, negative):
    ...
    pars = model.make_params(
        amplitude=amp, centerx=centerx, centery=centery, sigmax=sigmax, sigmay=sigmay
    )
...
class ComplexConstantModel(Model):
    ...
    def __init__(
        self, independent_vars=["x"], prefix="", nan_policy="raise", name=None, **kwargs
    ):
        kwargs.update(
            {
                "prefix": prefix,
...
class PolynomialModel(Model):
    ...
    def __init__(
        self, degree=7, independent_vars=["x"], prefix="", nan_policy="raise", **kwargs
    ):
        kwargs.update(
            {
                "prefix": prefix,
...
class SineModel(Model):
    ...
    def _set_paramhints_prefix(self):
        ...
        self.set_param_hint("shift", min=-tau - 1.0e-5, max=tau + 1.0e-5)
    ...
    def guess(self, data, x, **kwargs):
        ...
        errors = [
            np.linalg.norm(
                self.eval(
                    x=x, amplitude=amplitude, frequency=frequency, shift=shift_guess
                )
                - data
            )
...
class Gaussian2dModel(Model):
    ...
    def _set_paramhints_prefix(self):
        ...
        fmt = (
            "{factor:.7f}*{prefix:s}amplitude/(max({tiny}, {prefix:s}sigmax)"
...
class InvLorentzianModel(Model):
    r"""A model based on a Lorentzian or Cauchy-Lorentz distribution function.
    ...
    def __init__(self, independent_vars=["x"], prefix="", nan_policy="raise", **kwargs):
        ...
        super().__init__(invlorentzian, **kwargs)
        self._set_paramhints_prefix()
    ...
    def _set_paramhints_prefix(self):
        ...
        self.set_param_hint("fwhm", expr=fwhm_expr(self))
        self.set_param_hint("height", expr=height_expr(self))
        self.set_param_hint("sigma", min=-0.1, max=0.1)
...
class SplitLorentzianModel(Model):
    ...
    def _set_paramhints_prefix(self):
        ...
        height_expr = (
            "2*{pre:s}amplitude/{0:.7f}/max({1:.7f}, ({pre:s}sigma+{pre:s}sigma_r))"
        )
        self.set_param_hint("sigma", min=0)
        self.set_param_hint("sigma_r", min=0)
        self.set_param_hint("fwhm", expr=fwhm_expr.format(pre=self.prefix))
        self.set_param_hint(
            "height", expr=height_expr.format(np.pi, tiny, pre=self.prefix)
        )
    ...
    def guess(self, data, x, negative=False, **kwargs):
        ...
        pars[f"{self.prefix}sigma_r"].set(
            value=sigma.value, min=sigma.min, max=sigma.max
        )
        return update_param_vals(pars, self.prefix, **kwargs)
...
class VoigtModel(Model):
    ...
    def guess(self, data, x, negative=False, **kwargs):
        """Estimate initial model parameter values from data."""
        pars = guess_from_peak(self, data, x, negative, ampscale=1.5, sigscale=0.65)
        return update_param_vals(pars, self.prefix, **kwargs)
...
class PseudoVoigtModel(Model):
    ...
    def _set_paramhints_prefix(self):
        ...
        self.set_param_hint("fwhm", expr=fwhm_expr(self))
        fmt = (
            "(((1-{prefix:s}fraction)*{prefix:s}amplitude)/"
...
class MoffatModel(Model):
    ...
    def _set_paramhints_prefix(self):
        ...
        self.set_param_hint(
            "fwhm",
            expr=f"2*{self.prefix}sigma*sqrt(2**(1.0/max(1e-3, {self.prefix}beta))-1)",
        )
        self.set_param_hint("height", expr=f"{self.prefix}amplitude")

    def guess(self, data, x, negative=False, **kwargs):
        """Estimate initial model parameter values from data."""
        pars = guess_from_peak(self, data, x, negative, ampscale=0.5, sigscale=1.0)
        return update_param_vals(pars, self.prefix, **kwargs)
...
class Pearson7Model(Model):
    ...
    def _set_paramhints_prefix(self):
        ...
        fmt = (
            "{prefix:s}amplitude * gamfcn({prefix:s}expon)/"
...
class LognormalModel(Model):
    ...
    def _set_paramhints_prefix(self):
        ...
        fmt = (
            "{prefix:s}amplitude/max({0}, ({prefix:s}sigma*sqrt(2*pi)))"
...
class DampedOscillatorModel(Model):
    ...
    def guess(self, data, x, negative=False, **kwargs):
        """Estimate initial model parameter values from data."""
        pars = guess_from_peak(self, data, x, negative, ampscale=0.1, sigscale=0.1)
        return update_param_vals(pars, self.prefix, **kwargs)
...
class DampedHarmonicOscillatorModel(Model):
    ...
    def _set_paramhints_prefix(self):
        ...
        self.set_param_hint("sigma", min=0)
        self.set_param_hint("gamma", min=1.0e-19)
        fmt = (
            "({prefix:s}amplitude*{prefix:s}sigma)/"
...
class DoniachModel(Model):
    ...
    def _set_paramhints_prefix(self):
        fmt = (
            "{prefix:s}amplitude/max({0}, ({prefix:s}sigma**(1-{prefix:s}gamma)))"
...
class PowerLawModel(Model):
    ...
    def guess(self, data, x, **kwargs):
        """Estimate initial model parameter values from data."""
        try:
            expon, amp = np.polyfit(np.log(x + 1.0e-14), np.log(data + 1.0e-14), 1)
        except TypeError:
            expon, amp = 1, np.log(abs(max(data) + 1.0e-9))
...
class ExponentialModel(Model):
    ...
    def guess(self, data, x, **kwargs):
        """Estimate initial model parameter values from data."""
        try:
            sval, oval = np.polyfit(x, np.log(abs(data) + 1.0e-15), 1)
        except TypeError:
            sval, oval = 1.0, np.log(abs(max(data) + 1.0e-9))
...
class StepModel(Model):
    ...
    def guess(self, data, x, **kwargs):
        ...
        pars = self.make_params(amplitude=(ymax - ymin), center=(xmax + xmin) / 2.0)
...
class RectangleModel(Model):
    ...
    def _set_paramhints_prefix(self):
        ...
        self.set_param_hint(
            "midpoint", expr=f"({self.prefix}center1+{self.prefix}center2)/2.0"
        )
...
class ExpressionModel(Model):
    ...
    def __init__(
        self, expr, independent_vars=None, init_script=None, nan_policy="raise", **kws
    ):
